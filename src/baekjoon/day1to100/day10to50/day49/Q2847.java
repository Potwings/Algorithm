package baekjoon.day1to100.day10to50.day49;
...
public class Q2847 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < N; i++) {
            ...
            int beforeIdx = i - 1;
            while (beforeIdx >= 0) {
                int beforeChange = scoreArr[beforeIdx];
                // 이전 인덱스가 0보다 클 경우 해당 값도 그 이전값보다 큰지 확인
                while (score <= scoreArr[beforeIdx]) {
                    // 현재 단계보다 점수가 크거나 같을 경우 이전값 감소
                    scoreArr[beforeIdx]--;
                    result++;
                }
                if (beforeChange == scoreArr[beforeIdx]) {
                    //점수 변화가 없었을 경우 loop 중단
                    break;
                }
                // 변화가 있었을 경우 이전 값을 score에 담아주고 그 이전값과 비교
                score = scoreArr[beforeIdx--];
            }
        }
